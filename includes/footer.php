<footer>
        <div class="container">
            <div class="row">
                <div class="col-md-3">
                    <h5>About</h5>
                    <p>Freshcery brings farm-fresh, organic groceries straight from local farms to your door. We cut out the middlemen so you get the freshest produce at fair prices &mdash; and farmers get the returns they deserve.</p>
                </div>
                <div class="col-md-3">
                    <h5>Links</h5>
                    <ul>
                        <li>
                            <a href="<?php echo APPURL; ?>/about.php">About</a>
                        </li>
                        <li>
                            <a href="<?php echo APPURL; ?>/contact.php">Contact Us</a>
                        </li>
                        <li>
                            <a href="<?php echo APPURL; ?>/faq.php">FAQ</a>
                        </li>
                        <li>
                            <a href="<?php echo APPURL; ?>/shop.php">Shop</a>
                        </li>
                    </ul>
                </div>
                <div class="col-md-3">
                     <h5>Contact</h5>
                     <ul>
                         <li>
                            <i class="fa fa-map-marker"></i> Rua Joaquim Pestana n.&ordm; 2, 9300-145 Câmara de Lobos, Madeira
                        </li>
                         <li>
                            <a href="tel:+351967150118"><i class="fa fa-phone"></i> +351 967 150 118</a>
                        </li>
                        <li>
                            <a href="mailto:support@freshcery.com"><i class="fa fa-envelope"></i> support@freshcery.com</a>
                         </li>
                     </ul>

                     <h5>Follow Us</h5>
                     <ul class="social">
                         <li>
                            <a href="https://facebook.com/freshcery" target="_blank" rel="noopener"><i class="fab fa-facebook-f"></i></a>
                         </li>
                         <li>
                            <a href="https://instagram.com/freshcery" target="_blank" rel="noopener"><i class="fab fa-instagram"></i></a>
                         </li>
                         <li>
                            <a href="https://youtube.com/@freshcery" target="_blank" rel="noopener"><i class="fab fa-youtube"></i></a>
                         </li>
                     </ul>
                </div>
                <div class="col-md-3">
                     <h5>Get Our App</h5>
                     <ul class="mb-0">
                         <li class="download-app">
                             <a href="#"><img src="<?php echo APPURL; ?>/assets/img/playstore.png"></a>
                         </li>
                         <li style="height: 200px">
                             <div class="mockup">
                                 <img src="<?php echo APPURL; ?>/assets/img/mockup.png">
                             </div>
                         </li>
                     </ul>
                </div>
            </div>
        </div>
        <p class="copyright">&copy; <?php echo date('Y'); ?> Freshcery | Groceries Organic Store. All rights reserved.</p>
    </footer>

    <script type="text/javascript" src="<?php echo APPURL; ?>/assets/js/jquery.js"></script>
    <script type="text/javascript" src="<?php echo APPURL; ?>/assets/js/jquery-migrate.js"></script>
    <script type="text/javascript" src="<?php echo APPURL; ?>/assets/packages/bootstrap/libraries/popper.js"></script>
    <script type="text/javascript" src="<?php echo APPURL; ?>/assets/packages/bootstrap/bootstrap.js"></script>
    <script type="text/javascript" src="<?php echo APPURL; ?>/assets/packages/o2system-ui/o2system-ui.js"></script>
    <script type="text/javascript" src="<?php echo APPURL; ?>/assets/packages/owl-carousel/owl-carousel.js"></script>
    <script type="text/javascript" src="<?php echo APPURL; ?>/assets/packages/cloudzoom/cloudzoom.js"></script>
    <script type="text/javascript" src="<?php echo APPURL; ?>/assets/packages/thumbelina/thumbelina.js"></script>
    <script type="text/javascript" src="<?php echo APPURL; ?>/assets/packages/bootstrap-touchspin/bootstrap-touchspin.js"></script>
    <script type="text/javascript" src="<?php echo APPURL; ?>/assets/js/theme.js"></script>
</body>
</html>